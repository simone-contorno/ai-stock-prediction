...
class DataPreprocessor:
    @staticmethod
    def clear_zero_values(data: pd.DataFrame, features: List[str], 
    ...
    def prepare_sequence_input_data(input_data: pd.DataFrame, target_data: pd.DataFrame, 
                                  shift: int, logger: Optional[logging.Logger] = None) -> np.ndarray:
        """
        Prepare sequence data for RNN prediction (input only).
